...
INSERT INTO SHARED_POSTS VALUES (2 , 10 , 0 );
...
SELECT * FROM USER WHERE USER.ID NOT IN (
    ...
    SELECT USER_ID FROM SHARED_POSTS WHERE POST_ID = 1
);

-- GET LIST OF USERS WHO DON'T OWN ANY POSTS
SELECT * FROM USER WHERE USER.ID NOT IN (
    SELECT USER_ID FROM POSTS
);

-- GET LIST OF USERS WHO DON'T OWN ANY POSTS OR DON'T HAVE ANY POSTS SHARED WITH THEM
SELECT * FROM USER WHERE USER.ID NOT IN (
    SELECT USER_ID FROM POSTS
) AND USER.ID NOT IN (
    SELECT USER_ID FROM SHARED_POSTS
);

ALTER TABLE SHARED_POSTS
    ADD COLUMN SHARED_TYPE VARCHAR(50)
    DEFAULT 'VIEWER';
...
UPDATE SHARED_POSTS
SET SHARED_DATE = DATE('now')
WHERE SHARED_DATE IS NULL;

UPDATE SHARED_POSTS
SET SHARED_TYPE = 'EDITOR'
WHERE USER_ID = 4;

-- GET LIST OF POST WHICH EITHER USER OWNS OR IS USER HAS EDITOR RIGHTS.
SELECT * FROM POSTS WHERE USER_ID = 4 OR ID IN (
    SELECT POST_ID FROM SHARED_POSTS WHERE USER_ID = 4 AND SHARED_TYPE = 'EDITOR'
    );

-- Get all posts along with the user details who created them.
SELECT USER.NAME, USER.EMAIL, POSTS.TITLE, POSTS.CONTENT
FROM USER
         JOIN POSTS ON USER.ID = POSTS.USER_ID;

-- Get all posts shared with a specific user (e.g., user with ID 3) along with the post details:
SELECT POSTS.TITLE, POSTS.CONTENT, SHARED_POSTS.SHARED_TYPE, SHARED_POSTS.SHARED_DATE
FROM POSTS
         JOIN SHARED_POSTS ON POSTS.ID = SHARED_POSTS.POST_ID
WHERE SHARED_POSTS.USER_ID = 3;

-- Get all users who have shared posts and the details of those posts:
SELECT USER.NAME, USER.EMAIL, POSTS.TITLE, POSTS.CONTENT
FROM USER
         JOIN SHARED_POSTS ON USER.ID = SHARED_POSTS.USER_ID
         JOIN POSTS ON SHARED_POSTS.POST_ID = POSTS.ID;

-- Get all users who have shared posts and the details of those posts: UNIQUE
SELECT USER.NAME, USER.EMAIL, POSTS.TITLE, POSTS.CONTENT
FROM USER
         JOIN SHARED_POSTS ON USER.ID = SHARED_POSTS.USER_ID
         JOIN POSTS ON SHARED_POSTS.POST_ID = POSTS.ID
...
-- GET ALL USERS WHO HAVE SHARED POSTS AND THE DETAILS OF THOSE POSTS: COUNT
SELECT USER.NAME, COUNT(POSTS.USER_ID) AS COUNT_POSTS , USER.EMAIL, POSTS.TITLE, POSTS.CONTENT
FROM USER
         JOIN SHARED_POSTS ON USER.ID = SHARED_POSTS.USER_ID
         JOIN POSTS ON SHARED_POSTS.POST_ID = POSTS.ID
GROUP BY USER.ID;

-- GET ALL USERS WHO HAVE CREATED POSTS AND THE DETAILS OF THOSE POSTS: COUNT
SELECT USER.NAME, COUNT(POSTS.USER_ID) AS COUNT_POSTS , USER.EMAIL, POSTS.TITLE, POSTS.CONTENT
FROM USER
         JOIN POSTS ON USER.ID = POSTS.USER_ID
GROUP BY USER.ID;

-- Get the list of users who do not own any posts but have posts shared with them:
SELECT USER.NAME, USER.EMAIL
FROM USER
         LEFT JOIN POSTS ON USER.ID = POSTS.USER_ID
WHERE POSTS.USER_ID IS NULL
  AND USER.ID IN (SELECT USER_ID FROM SHARED_POSTS);
